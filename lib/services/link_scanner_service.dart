...
class LinkScannerService {
  ...
  static Future<ScanResult> scanUrl(String url) async {
    ...
    return ScanResult(url: url, verdict: Verdict.safe, reason: 'No threats found');
  }
  ...
  static ScanResult _localCheck(String url) {
    ...
    if (RegExp(r'^\d+\.\d+\.\d+\.\d+$').hasMatch(host)) {
      return ScanResult(
          url: url,
          verdict: Verdict.dangerous,
          reason: 'URL uses an IP address instead of a domain name — common in scams',
    ...
    if (fakeBrands.any((b) => host.contains(b))) {
      return ScanResult(
            url: url,
            verdict: Verdict.dangerous,
            reason: 'This link is pretending to be a real brand but uses leetspeak characters',
            userWarning: '⚠️ Fake link — possible scam');
    }

    // 4. Phishing keywords in path
    final phishingWords = [
      'verify', 'kyc', 'otp', 'login', 'signin', 'account',
      'suspend', 'confirm', 'update', 'secure', 'wallet',
      'prize', 'winner', 'claim', 'reward', 'free-money',
      'urgent', 'blocked', 'alert'
    ];
    ...
    // 5. Suspicious TLDs
    const suspiciousTlds = ['.xyz', '.tk', '.ml', '.cf', '.ga', '.gq', '.top', '.click'];
    ...
    return ScanResult(url: url, verdict: Verdict.unknown, reason: 'Needs API check');
  }

  // ─── LAYER 2: GOOGLE SAFE BROWSING ────────────────────────────────────────

  static Future<bool> _callSafeBrowsing(String url) async {
    const apiKey = Secrets.safeBrowsingKey; // your existing secrets.dart key
    ...
    if (response.statusCode == 200) {
      ...
      return data.containsKey('matches') && (data['matches'] as List).isNotEmpty;
    }
    return false;
  }
}
